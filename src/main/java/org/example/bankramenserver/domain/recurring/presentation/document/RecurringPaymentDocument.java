...
@Tag(
        name = "정기결제 API",
        description = "정기결제 등록, 목록 조회, 삭제, 자동 감지 후보 확정 API"
)
public interface RecurringPaymentDocument {

    @Operation(
            summary = "정기결제 목록 조회",
            description = "사용자의 활성 정기결제 목록과 이번 달 예정된 정기결제 총 결제 금액을 조회합니다."
    )
    RecurringPaymentListResponse getRecurringPayments();

    @Operation(
            summary = "정기결제 직접 등록",
            description = "사용자가 거래 기록을 기준으로 정기결제를 직접 등록합니다."
    )
    CreateRecurringPaymentResponse create(
    ...
    ConfirmRecurringPaymentResponse confirm(
    ...
    DeleteRecurringPaymentResponse delete(
